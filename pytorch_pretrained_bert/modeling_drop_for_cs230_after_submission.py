...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    print('picking model from modeling_count.py')                               ## added to assure right script is selected
...
class BertForQuestionAnswering_count(BertPreTrainedModel):                      ## change name


    print('Now picking BertForQuestionAnswering_count... from cache')           ## Added print

    def __init__(self, config, max_seq_length, add_sub_max_length):             ### Added max_seq_length and add_sub_max_length
        super(BertForQuestionAnswering_count, self).__init__(config)            ## Changed model name
        self.bert = BertModel(config)
        self.max_seq_length = max_seq_length                                    ## Added max_seq_length
        self.config_hidden_size = config.hidden_size                            ### Added
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None, answer_types=None,
                    answers_as_add_sub_expressions=None, answers_as_add_sub_numbers=None, add_sub_mask=None, answers_as_counts=None):     ### added tensors
        ...
        answers_as_counts_preds = torch.argmax(class_logits, -1).long()               ## Added argmax to get answers_as_counts predictions
        #print("raw answers_as_counts_preds: ", answers_as_counts_preds)
        ...
        #print("add_sub_logits nach view: ", add_sub_logits.size())
        add_sub_logits = self.classification_4(add_sub_logits)
        #print("add_sub_logits nach 2. layer: ", add_sub_logits.size())
        #add_sub_logits = add_sub_logits.view(add_sub_logits.size(0),add_sub_logits.size(2),add_sub_logits.size(1))
        #print("add_sub_logits nach view: ", add_sub_logits.size())
        answers_as_add_sub_expressions_preds = torch.argmax(add_sub_logits, 1).float()
        answers_as_add_sub_expressions_preds[answers_as_add_sub_expressions_preds==2] = -1
        #print("answer_as_add_sub_expressions_preds: ", answer_as_add_sub_expressions_preds.size())
        ...
        answer_type_input = sequence_output.view(sequence_output.size()[0], -1)    ### Reshape into (batch, hidden_size x max_seq_length)
        answer_type_logits = self.classification_5(answer_type_input)              ### start Added...
        answer_type_logits = self.relu(answer_type_logits)
        answer_type_logits = self.classification_6(answer_type_logits)
        #print("answer_type_logits: ", answer_type_logits.size())
        answer_type_selector = torch.argmax(answer_type_logits, -1, keepdim=True).long()
        #print("raw answer_type_selector: ", answer_type_selector)
        ...
        #print("answer_types: ", answer_types.size())
        #print("raw answer_types: ", answer_types)                                   ###... end Added
        ...
        #print('shape of answers_as_add_sub_numbers: ', answers_as_add_sub_numbers.size())      ### Added print
        #print('raw answers_as_add_sub_numbers: ', answers_as_add_sub_numbers)  ### Added print
        #print('shape of add_sub_input: ', add_sub_input.size())                ### Added print
