...
class SquadExample(object):
    ...
    def __repr__(self):
        ...
        if self.answer_type:
            s += ", answer_type: %r" % (self.answer_type)                                         ### added
        if self.answer_as_add_sub_expressions:
            s += ", answer_as_add_sub_expressions: %r" % (self.answer_as_add_sub_expressions)     ### added
        if self.answer_as_add_sub_numbers:
            s += ", answer_as_add_sub_numbers: %r" % (self.answer_as_add_sub_numbers)             ### added
        if self.add_sub_mask:
            s += ", add_sub_mask: %r" % (self.add_sub_mask)                                       ### added
...
def read_squad_examples(input_file, is_training, version_2_with_negative, add_sub_max_length):  ### added add_sub_max_length
    ...
    counter_1 = 0
    counter_0 = 0
    count_answer_as_count_minus1 = 0
    ...
    #print("count_answer_as_count_minus1: ", count_answer_as_count_minus1)
    return examples
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if is_training and example.is_impossible:
                start_position = 0
                end_position = 0
            if example_index < 0:                                               ## 0 changed from 20
                logger.info("*** Example ***")
...
RawResult = collections.namedtuple("RawResult",
                                   ["unique_id", "start_logits", "end_logits", "alt_answer"])      ### added alt_answer
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        #print("example_index :", example_index)                                 ## added print
...
def main():
    ...
    parser.add_argument("--add_sub_max_length", default=20, type=int,
                        help="The maximum number of numbers in the passage we consider.")
    ...
    # Prepare model
    model = BertForQuestionAnswering_count.from_pretrained(args.bert_model, max_seq_length=args.max_seq_length, add_sub_max_length=args.add_sub_max_length,    ### change name and add max_seq_length, add_sub_max_length
    ...
    if args.do_train:
        ...
        #train_examples = train_examples[:1000]                                                                   ## Reduce number of train examples to ????
        ...
        all_answer_types = torch.tensor([f.answer_type for f in train_features], dtype=torch.long)                ### added all_answer_types tensor long
        all_answers_as_add_sub_expressions = torch.tensor([f.answer_as_add_sub_expressions for f in train_features], dtype=torch.long)     ### added answers_as_add_sub_expressions tensor long
        all_answers_as_add_sub_numbers = torch.tensor([f.answer_as_add_sub_numbers for f in train_features], dtype=torch.float)     ### added answers_as_add_sub_numbers tensor float
        all_add_sub_mask = torch.tensor([f.add_sub_mask for f in train_features], dtype=torch.long)              ### added add_sub_mask tensor long
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                   all_start_positions, all_end_positions, all_answer_types, all_answers_as_add_sub_expressions, all_answers_as_add_sub_numbers, all_add_sub_mask, all_answers_as_counts)   ### added tensors
        ...
        #hits_s = []                                                                                    ## Added for "Tensorboard"
        #hits_c = []                                                                                    ## Added for "Tensorboard"
        #hits_t = []                                                                                    ## Added for "Tensorboard"

        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])):
                if n_gpu == 1:
                    batch = tuple(t.to(device) for t in batch) # multi-gpu does scattering it-self
                input_ids, input_mask, segment_ids, start_positions, end_positions, answer_types, answers_as_add_sub_expressions, answers_as_add_sub_numbers, add_sub_mask, answers_as_counts = batch  ### added tensors
                loss = model(input_ids, segment_ids, input_mask, start_positions, end_positions, answer_types, answers_as_add_sub_expressions, answers_as_add_sub_numbers, add_sub_mask, answers_as_counts)    ### added tensors

                #s=hits_signs.tolist()
                #c=hits_counts.tolist()
                #t=hits_type.tolist()
                #print("s, c, t: ", s,c,t)
                #hits_s += s                                                     ## Added
                #hits_c += c                                                     ## Added
            ...
            #data = np.column_stack((hits_s, hits_c, hits_t))         ## Added
            #np.savetxt('basic_indicators1002.out', data)                                 ## Added for graphs
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = BertForQuestionAnswering_count.from_pretrained(args.output_dir, args.max_seq_length, args.add_sub_max_length)    ### Changed name & Added max_seq_length and add_sub_max_length
        tokenizer = BertTokenizer.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
    else:
        model = BertForQuestionAnswering_count.from_pretrained(args.bert_model, args.max_seq_length, args.add_sub_max_length)    ### Changed name & Added max_seq_length and add_sub_max_length
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        #eval_examples = eval_examples[:150]                                                             ## Reduce number of dev examples to ????
        ...
        all_answers_as_add_sub_numbers = torch.tensor([f.answer_as_add_sub_numbers for f in eval_features], dtype=torch.float)     ### added answers_as_add_sub_numbers tensor float
        all_example_index = torch.arange(all_input_ids.size(0), dtype=torch.long)
        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_answers_as_add_sub_numbers, all_example_index)    ### added all_answers_as_add_sub_numbers
        ...
        for input_ids, input_mask, segment_ids, answers_as_add_sub_numbers, example_indices in tqdm(eval_dataloader, desc="Evaluating", disable=args.local_rank not in [-1, 0]):    ### added answers_as_add_sub_numbers
            if len(all_results) % 1000 == 0:
                logger.info("Processing example: %d" % (len(all_results)))
            input_ids = input_ids.to(device)
            input_mask = input_mask.to(device)
            segment_ids = segment_ids.to(device)
            answers_as_add_sub_numbers = answers_as_add_sub_numbers.to(device)  ### added
            with torch.no_grad():
                batch_start_logits, batch_end_logits, batch_alt_answers = model(input_ids,                                  ### added batch_alt_answers
                                        segment_ids, input_mask, answers_as_add_sub_numbers=answers_as_add_sub_numbers)
